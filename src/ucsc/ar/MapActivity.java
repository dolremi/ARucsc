...
public class MapActivity extends FragmentActivity implements
	...
	private LatLng myPosition = new LatLng(0, 0);// won't actually be displayed
													// as (0,0). Will be changed
													// as soon as
													// onLocationChanged is
													// called. More efficient
													// than setting to null and
													// checking whether it has
													// received data.
	private long lastTimeMapUpdated = 0;

	private static final double MAX_DISTANCE = 30000;// 30km . In meters.
														// Crashes for long
														// distances, e.g.
														// Oxford-Bangalor
	private static final int MIN_REFRESH_PERIOD = 30000;// 30s. In millisecond.
														// Will request for new
														// data automaticly only
														// if MIN_REFRESH_PERIOD
			...
			public void onClick(View v) {
				if (gd != null && mDoc != null) {
					gd.animateDirection(map, gd.getDirection(mDoc),
							GoogleDirection.SPEED_NORMAL, true, false, true,
							false, null, false, true, null);
				} else if (gd == null)
					Log.d("MapActivity", "gd==null");
				else if (mDoc == null)
					Log.d("MapActivity", "mDoc==null");
			}
		});
	...
	private void poisSetup() {
		for (int i = 0; i < names.length; i++) {
			...
			map.addMarker(new MarkerOptions()
					.position(new LatLng(lats[i], longs[i])).title(names[i])
	...
	private void drawRouteOnMap() {
		...
		try {
			if (distance(myPosition.latitude, myPosition.longitude,
					destPosition.latitude, destPosition.longitude) <= MAX_DISTANCE) {
				gd.setLogging(true);
				gd.request(myPosition, destPosition,
	...
	public double distance(double lat_a, double lng_a, double lat_b,
			double lng_b) {
