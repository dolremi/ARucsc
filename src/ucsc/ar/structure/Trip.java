...
public class Trip {
	private List<Pinterest> Pinterests = new ArrayList<Pinterest>();//All the Pinterests in this Trip
	...
	/** Used only for the preview as there will be difference between the size field and Pinterests.size.
	 * 
	 * @param Pinterests
	 * @param id
	 * @param name
	 ...
	 * @param size
	 */
	public Trip(List<Pinterest> Pinterests, String id, String name, String info, int size){
	...
	/**Creates a Trip with the following parameters. If the array is null or empty creates it so that it has a single Pinterest("Empty Trip", 0.0, 0.0, "Empty Trip")
	 * 
	 * @param Pinterests
	 * @param id
	 * @param name
	 * @param info
	 */
	//If the array is empty it creates a Trip with a single Pinterest
	...
	/**Creates a Trip with the following parameters. If the ArrayList is null or empty creates it so that it has a single Pinterest("Empty Trip", 0.0, 0.0, "Empty Trip")
	 * 
	 * @param Pinterests
	 * @param id
	 * @param name
	...
	public Trip(List<Pinterest> Pinterests, String id, String name, String info){
		...
		if (Pinterests!=null && Pinterests.size()!=0){
			...
			this.Pinterests.add(new Pinterest("Empty Trip", 0.0, 0.0, "Empty Trip"));
		}
		this.size = this.Pinterests.size();
		System.out.println("Trip "+id+" of size "+this.Pinterests.size()+" created");
	}
	
	/**Creates a Trip using an InputStream from an xml file.
	 * Format of the xml:
	 * @param fileName
	 */
	...
	/**Returns the Pinterest at that index. If the index is invalid will return the first Pinterest of the Trip.
	 * 
	 * @param ind
	 ...
	 */
	//Increments index (mod Pinterests.length) and returns the Pinterest with that index
	public Pinterest moveToNextPinterest(){
		index = (index+1)%size;
		if (index==0) System.out.println("Trip finished");
		return Pinterests.get(index);
	}
	
	/** Returns the Pinterest in the next position(mod size). Doesn't increment the index. 
	 * Will print out "No next Pinterest" if it has wrapped around.
	 * 
	 * @return the Pinterest in the next position
	 */
	public Pinterest getNextPinterest(){
		int nextIndex = (index+1)%Pinterests.size();
		if (nextIndex==0) System.out.println("No next Pinterest.");
